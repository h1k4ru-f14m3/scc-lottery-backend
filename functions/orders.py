...
class OrderManager():
    ...
    def add_tickets_to_cart(self, ticket_id, user_session, cart_session):
        ...
        """Return the Data"""
        return glvars.ReturnData(True, 'Added ticket!', cart=cart.to_dict(), user=user.to_dict()).send()
    ...
    def remove_tickets_from_cart(self, ticket_id, user_session, cart_session):
        ...
        """Return the Data"""
        return glvars.ReturnData(True, 'Removed ticket!', cart=cart.to_dict(), user=user.to_dict()).send()
    ...
    def confirm_bought(self, img_data, user_session, cart_session):
        ...
        return glvars.ReturnData(True, 'Tickets in cart bought.', cart=None, user=user.to_dict(), order_id=order_id).send()
    ...
    def confirm_cart(self, order_id):
        ...
        cart_edit = self.db_man.edit_row(glvars.orders_table,
                                         ('id',),
                                         (order_id,),
                                         ('confirmed',),
                                         (1,))
    ...
    def save_to_db(self, user_session, cart_session):
        ...
        response = self.db_man.edit_row(glvars.orders_table, ('id',), 
                                        (cart.id,), 
